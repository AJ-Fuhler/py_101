...
for index in range(10):
    flintstones_str = '-' + flintstones_str    
    print(flintstones_str)
# or, better:
...
# 2. Alan wrote the following function, which was intended to return all of the
#    factors of number. Alyssa noticed that this code would fail when the input
#    is a negative number, and asked Alan to change the loop. How can he make
#    this work? Note that we're not looking to find the factors for negative
#    numbers, but we want to handle it gracefully instead of going into an
#    infinite loop.
...
# 3. Alyssa was asked to write an implementation of a rolling buffer. You can
...
# The difference is that the first function mutates the original object.
# The second function initializes a new local variable and returns that instead.



# 4. What will the following two lines of code output?

print(0.3 + 0.6) # not exactly 0.9
...
# False, == operater doesn't work to determine whether a value is nan
...
# Yes, value access the key's value, which is a dictionary object. value["age"]
# += 42 directly alters the original, since demo_dict was not reassigned.
# dictionaries are mutable, so when passed as an argument to a function, the
# original will be mutated unless reassignment happens within the function.



# 8. Function and method calls can take expressions as arguments. Suppose we
...
# 10. In Python, every object has a unique identifier that can be accessed
...
# True integers from -5 to 256 have pre-assigned memory locations. When you
# reference an integer in that range, Python will point to the same memory
# spot. This is a process called interning, which improves performance.